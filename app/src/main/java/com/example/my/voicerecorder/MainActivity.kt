...
class MainActivity : AppCompatActivity() {
    ...
    private var recordingAnimation: LottieAnimationView? = null
    private var nameArea: View? = null
    private var nameField: EditText? = null
    private var confirmNameButton: ImageView? = null

    private var recordsList: RecyclerView? = null
    private var recordsArray: ArrayList<Record> = ArrayList()
    ...
    private fun getRecords() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val fileName =
                    cursor.getString(cursor.getColumnIndex(DatabaseHelper.COLUMN_FILE_NAME))
    ...
    private fun record() {
        if (recordingEnabled) {
            ...
            mediaRecorder = null
            mainButton?.setImageResource(R.drawable.voice)
    ...
    private fun save() {
        val id = Utils.getID()
        im?.hideSoftInputFromWindow(nameField?.windowToken, 0)

        val newRecord =
            Record(id, nameField?.text?.toString()!!, currentFileName, Utils.getCurrentDate())
